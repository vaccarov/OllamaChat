...
export async function listDocuments(chatServerUrl: string, embeddingModel?: string, chatId?: string): Promise<RagDocument[]> {
  const params = new URLSearchParams();
  if (embeddingModel) params.append('embedding_model', embeddingModel);
  if (chatId) params.append('chat_id', chatId);

  const response = await fetch(`${chatServerUrl}/documents/list?${params.toString()}`, {
    method: 'GET',
  });
...
export async function searchDocuments(chatServerUrl: string, query: string, embeddingModel: string, chatId: string): Promise<RagDocument[]> {
...
export async function resetAllDocuments(chatServerUrl: string, embeddingModel: string): Promise<{ message: string }> {
  const response = await fetch(`${chatServerUrl}/documents/reset/${embeddingModel}`, {
    method: 'DELETE',
  });
